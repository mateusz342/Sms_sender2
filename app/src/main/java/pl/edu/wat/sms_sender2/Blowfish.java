...
public class Blowfish {
   ...
   // byte key=(byte)0x2FFD72DB;
    ...
    private final void BF_encrypt (long L, long R, long[] out, int outOff) {
        long a, b, c, d;
        ...
        for (int i = 0; i < rounds; ) {
            ...
            c = 0x200 | ((L >>>  8) & 0xFF);		// 512 +
            d = 0x300 |   L         & 0xFF;			// 768 +
            R ^= (((sKey[(int) a] + sKey[(int) b]) ^ sKey[(int) c]) + sKey[(int) d]) ^ P[++i];

            a =          (R >>> 24) & 0xFF;
            b = 0x100 | ((R >>> 16) & 0xFF);		// 256 +
            c = 0x200 | ((R >>>  8) & 0xFF);		// 512 +
            d = 0x300 |   R         & 0xFF;			// 768 +
            L ^= (((sKey[(int)a] + sKey[(int)b]) ^ sKey[(int)c]) + sKey[(int)d]) ^ P[++i];
        }
        R ^= P[rounds + 1];
    ...
    public  byte[] intToByteArray(long key) {
        return new byte[] {
                (byte)((key >>> 24) & 0xFF),
                (byte)((key >>> 16)& 0xFF),
                (byte)((key >>> 8)& 0xFF),
